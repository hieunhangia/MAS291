namespace Question_1
{
    ...
    {
        public static void Result(double p, int n, int loop)
        {
        ...
        private static double ExpectedMean(double p, int n) => 1 + 2 * (n - 1) * p * (1 - p);

        private static double ExpectedVariance(double p, int n)
            => 2 * p * (1 - p) * (2 * n - 3 - 2 * p * (1 - p) * (3 * n - 5));

        private static void ActualMeanAndVariance(double p, int n, int loop, out double mean ,out double variance)
        {
        ...
        private static int Runs(double p, int n)
        {
            var rd = new Random();
